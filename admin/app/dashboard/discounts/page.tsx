...
import { getDiscounts, deleteDiscount, updateDiscount } from "@/lib/api";
...
export default function DiscountsPage() {
  ...
  async function toggleActive(d: Discount) {
    ...
    await updateDiscount(d.id, { active: !d.active }).catch(() => {});
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <Link href="/dashboard/discounts/new" className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#0B3D33] text-white rounded-xl text-sm font-semibold hover:opacity-90 transition-opacity">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-100">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search codes…" className="w-full pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#C9A96E]/30" />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            {!search && <Link href="/dashboard/discounts/new" className="inline-block mt-2 text-sm text-[#C9A96E] hover:underline">Create your first discount →</Link>}
          </div>
        ) : (
